 ...
 * authentication on custom domains and other hosts where the Frontend API
 * hostname would not get a valid browser-trusted certificate without a proxy.
 *
 * AUTH CONFIGURATION: Manage users, login providers, branding, and OAuth in
 * the Clerk Dashboard (https://dashboard.clerk.com/) for your application.
 *
 * IMPORTANT:
...
// IMPORTANT — if you change this path, update it in two other places:
//   1. artifacts/same-same/app/_layout.tsx  — CLERK_PROXY_URL_FALLBACK constant
//   2. artifacts/same-same/eas.json         — EXPO_PUBLIC_CLERK_PROXY_URL in preview/production profiles
