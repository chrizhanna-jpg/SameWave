...
// SameWave £1 one-time, lifetime unlock. Modelled as a non-consumable IAP
// (Google Play "managed product" / Apple "non-consumable") because the
// purchase is permanent and never expires — not a subscription, not a
// consumable. Same product identifier across stores so the client never
// has to branch by platform.
const PROJECT_NAME = "SameWave";

const PRODUCT_IDENTIFIER = "samewave_pro";
// Non-consumable Play Store products do NOT use the
// `productId:basePlanId` format (that's subscriptions only) — bare SKU.
const PLAY_STORE_PRODUCT_IDENTIFIER = "samewave_pro";

const PRODUCT_DISPLAY_NAME = "SameWave Pro";
const PRODUCT_USER_FACING_TITLE = "SameWave Pro";

const APP_STORE_APP_NAME = "SameWave (iOS)";
const APP_STORE_BUNDLE_ID = "echo.samewaveripple.app";
const PLAY_STORE_APP_NAME = "SameWave (Android)";
const PLAY_STORE_PACKAGE_NAME = "echo.samewaveripple.app";
...
// Custom package key — the standard `$rc_*` keys are subscription-shaped
// (monthly/annual/lifetime). `lifetime` is fine as a custom key for our
// single non-consumable product.
const PACKAGE_IDENTIFIER = "lifetime";
const PACKAGE_DISPLAY_NAME = "SameWave Pro · Lifetime";

// £1.00 — micros = pounds * 1,000,000. Test Store prices are immutable
// once set; production prices come from Play Console / App Store Connect.
const PRODUCT_PRICES = [{ amount_micros: 1_000_000, currency: "GBP" }];
...
async function seedRevenueCat() {
  ...
  const ensureProductForApp = async (
    ...
    const body: CreateProductData["body"] = {
      ...
      app_id: targetApp.id,
      // Non-consumable: bought once, unlocks forever. No `subscription`
      // field — that's only for subscription-typed products.
      type: "non_consumable",
    ...
    if (error) {
      console.error("createProduct error for " + label + ":", JSON.stringify(error, null, 2));
  ...
  if (priceError) {
    if (
      ...
      console.error("Test store prices error:", JSON.stringify(priceError, null, 2));
